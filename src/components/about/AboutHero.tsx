...
export function AboutHero({ profile }: AboutHeroProps) {
  ...
  const rolesString = profile.roles
    ? profile.roles.map((r) => r.role).join(' • ')
    : ''

  const avatarUrl = typeof profile.avatar === 'object' && profile.avatar !== null
    ? profile.avatar.url || ''
    : ''
  const avatarAlt = typeof profile.avatar === 'object' && profile.avatar !== null

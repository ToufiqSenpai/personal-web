  ...
  return (
    <section className="px-6 py-12">
      <div className="mx-auto max-w-4xl">
        <Reveal>
          <p className="mb-2 font-mono text-xs uppercase tracking-wider text-accent">Life Off-Screen</p>
          <h2 className="mb-8 text-2xl font-bold text-ink">Hobbies & Interests</h2>
        </Reveal>
          ...
          {hobbiesItems.map((hobby, index) => {
            ...
            return (
              <Reveal
                key={index}
                delay={index * 90}
                className={`group relative overflow-hidden rounded-2xl bg-surface ${layout.col} ${layout.row} ${layout.minH}`}
              >
                <div
                  className={`absolute inset-0 transition-transform duration-500 ease-out group-hover:scale-[1.06] ${tiltClass} motion-reduce:transform-none motion-reduce:transition-none`}
                >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <h3 className="mb-1 text-lg font-bold text-white transition-transform duration-300 group-hover:-translate-y-1">
                    {hobby.title}
                  </h3>
                  {hobby.description && (
                    <p className="max-h-0 overflow-hidden text-sm text-white/80 opacity-0 transition-all duration-300 group-hover:max-h-20 group-hover:opacity-100">
                      {hobby.description}
                    </p>
                  )}
                </div>

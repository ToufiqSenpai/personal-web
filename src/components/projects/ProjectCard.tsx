...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <div className="group relative flex flex-col rounded-xl border border-border bg-surface overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-accent">
      <div className="relative aspect-video w-full overflow-hidden border-b border-border bg-muted/10">
        ...
        {(project as any).projectType === 'public' && (project.demoUrl || (project as any).repositoryUrl) && (
          <div className="absolute inset-0 bg-ink/70 opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex items-center justify-center gap-4 backdrop-blur-sm">
            {project.demoUrl && (
              <a
                href={project.demoUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-full bg-surface text-ink px-4 py-2 font-medium text-sm hover:scale-105 transition-transform flex items-center gap-2"
              >
      ...
      <div className="flex flex-col flex-1 p-5">
        <h3 className="font-mono text-lg font-bold mb-2 text-ink group-hover:text-accent transition-colors">
          {project.title}
        </h3>
